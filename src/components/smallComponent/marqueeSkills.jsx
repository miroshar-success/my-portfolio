...
const MarqueeSkills = ({skills, direction="left"}) => {
  return (
    <div className="flex gap-7 md:gap-12 bg-gradient-to-t from-black via-slate-700 to-black text-pink-500 ">
      <div className={`${direction === 'left' ? '' : 'right-'}marquee-group flex shrink-0 items-center justify-around gap-7 md:gap-12 min-w-full`}>
        {" "}
      ...
      <div
        aria-hidden="true"
        className={`${direction === 'left' ? '' : 'right-'}marquee-group flex shrink-0 items-center justify-around gap-7 md:gap-12 min-w-full`}
      >
        {" "}
        {skills.map(({name, tag}, idx) => {
          const TagComponent = tag;
